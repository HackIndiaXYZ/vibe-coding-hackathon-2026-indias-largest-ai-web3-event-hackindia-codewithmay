...
export class LAppModel extends CubismUserModel {
  ...
  private setupModel(setting: ICubismModelSetting): void {
    ...
    const setupLipSyncIds = (): void => {
      ...
      // Use AmplitudeProvider (fed from Web Audio / simulation) instead of
      // LAppWavFileHandler, which only works with WAV files served over HTTP.
      if (this._lipSyncIds.length > 0) {
        const lipSyncUpdater = new CubismLipSyncUpdater(
          this._lipSyncIds,
          this._amplitudeProvider
        );
  ...
  public update(): void {
    ...
    // Post-scheduler hook — called AFTER effects (lip sync, blink) but BEFORE the final
    // _model.update() GPU push. This is the only safe window to setParameterValueById
    // without being wiped by loadParameters() at the start of the next update cycle.
    if (this._postSchedulerHook) {
      this._postSchedulerHook(this._model, deltaTimeSeconds);
    }
  ...
  /**
   * Register a callback that runs after the update scheduler (effects/lip sync)
   * but before the final model.update() GPU push each frame.
   * Use this to drive parameters that must not be overwritten by loadParameters().
   */
  public setPostSchedulerHook(
    hook: ((model: CubismModel, dt: number) => void) | null
  ): void {
    this._postSchedulerHook = hook;
  }
  ...
  /**
   * Wire the real-time amplitude ref into the AmplitudeProvider so that
   * CubismLipSyncUpdater uses actual audio energy for mouth animation.
   * Call this once after construction, before the model finishes loading.
   */
  public setAmplitudeRef(ref: { current: number }): void {
    // Replace the internal provider with a new one bound to the ref.
    // The provider is already registered in the scheduler; we mutate it in place
    // by creating a new AmplitudeProvider and replacing the field — the scheduler
    // holds a reference to the old object, so we update via the existing instance's
    // internal ref instead.
    (this._amplitudeProvider as any)._amplitudeRef = ref;
    (this._amplitudeProvider as any)._smoothed = 0;
  }
  ...
  public constructor() {
    ...
    this._amplitudeProvider = new AmplitudeProvider({ current: 0 });
  ...
  _amplitudeProvider: AmplitudeProvider; // Real-time amplitude provider for browser TTS lip sync
  _postSchedulerHook: ((model: CubismModel, dt: number) => void) | null = null;
