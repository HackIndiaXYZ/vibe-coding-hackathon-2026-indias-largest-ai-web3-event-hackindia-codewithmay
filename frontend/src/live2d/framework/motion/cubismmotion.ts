...
export class CubismMotion extends ACubismMotion {
  ...
  public parse(
    ...
    if (json.isExistMotionFadeInTime()) {
      this._fadeInSeconds =
        json.getMotionFadeInTime() < 0.0 ? 0.3 : json.getMotionFadeInTime();
    } else {
      this._fadeInSeconds = 0.3;
    }

    if (json.isExistMotionFadeOutTime()) {
      this._fadeOutSeconds =
        json.getMotionFadeOutTime() < 0.0 ? 0.3 : json.getMotionFadeOutTime();
    } else {
      this._fadeOutSeconds = 0.3;
    }
